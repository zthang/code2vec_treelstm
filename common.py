...
class common:
    ...
    # 对count小于min_count的值不进行记录，返回word_to_count next_index记录字典中的word个数
    @staticmethod
    def _load_vocab_from_histogram(path, min_count=0, start_from=0, return_counts=False):
        with open(path, 'r') as file:
            ...
            for line in file:
                ...
                # 出现次数小于min_count的就不记录在word_to_count中了
                if count < min_count:
                    continue
    ...
    def load_vocab_from_histogram(path, min_count=0, start_from=0, max_size=None, return_counts=False):
        if max_size is not None:
            ...
            if next_index <= max_size:
                ...
                return results
            # Take min_count to be one plus the count of the max_size'th word 按count大小排序 取提及次数高的进行存储 从而会导致一些word不在word_to_count中
