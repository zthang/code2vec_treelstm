...
//			}
			...
			Set<Integer> non_vis = new HashSet<Integer>();
			...
			while(!non_vis.isEmpty()) {
				...
				while(!q.isEmpty()) {
					int cur = q.poll();
					non_vis.remove(cur);
					for(int x:g[cur]) g[x].remove(cur);
					for(int x:non_vis) if(!g[cur].contains(x)) q.add(x);
				}
				int max = -1;
				int max_idx = -1;
				for(int x:non_vis) {
					if(non_vis.size()-g[x].size()>max) {
						max = non_vis.size()-g[x].size();
						max_idx = x;
					}
				}
				start = max_idx;
