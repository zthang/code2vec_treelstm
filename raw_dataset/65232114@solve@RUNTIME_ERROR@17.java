		void solve(int testNumber, FastScanner s, PrintWriter out) {
			...
			while(IDX < K - 1) {
				...
				if(vtx != cur.origin && vroot != vtx && vroot != root) { // we're linking up! 
					int old = root ^ vroot, god;
					cdist += dist[vroot].get(vtx); // distance back to the station
					// CRITICAL JOIN!
					unions[IDX][0] = root; unions[IDX][1] = vroot; limits[IDX++] = cdist;
					DSU.union(root, vroot); old ^= god = DSU.find(cur.origin);
					// bring in the new neighbors!
					for(Entry<Integer, Long> E : dist[old].entrySet())
						dist[god].put(E.getKey(), min(dist[god].getOrDefault(E.getKey(), oo), E.getValue()));
					dist[old].clear();
					continue; // the station at vtx has already searched neighbors, or will
				}
				
				DSU.union(root, vtx); // bring 'em to our side
