public void solve(InputReader in, PrintWriter out) throws IOException {
    ...
    HashMap<Long, Integer> groupFinder = new HashMap<Long, Integer>();
    ...
    long target = sum / k;
    HashMap<Long, Long> nxt = new HashMap<Long, Long>();
    for (int i = 0; i < k; i++) {
        for (long j : arr[i]) {
    ...
    HashMap<Integer, ArrayList<Long>> cyc = new HashMap<Integer, ArrayList<Long>>();
    for (int i = 0; i < k; i++) {
        for (long j : arr[i]) {
