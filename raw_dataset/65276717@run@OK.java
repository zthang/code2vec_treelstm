public void run() throws Exception {
    ...
    int asdf = 0;
    for (int i = 0; i < k; i++) for (int j = 0; j < vals[i].length; j++) {
        long cur = vals[i][j];
    ...
    for (int i = 1; i < pos.length; i++) {
        to[i] = -1;
        for (int s = i; s != 0; s = (s - 1) & i) if (exists[s] && pos[i - s]) {
            pos[i] = true;
            to[i] = s;
            break;
        }
    ...
    if (pos[(1 << k) - 1]) {
        ...
        for (int i = 0; i < actualmoves.length; i += 2) out.println(actualmoves[i] + " " + actualmoves[i + 1]);
    } else
        out.println("No");
    // /
    out.flush();
