public void solve(int testNumber, FastReader s, PrintWriter w) {
    ...
    ArrayList<pair<Integer, Long>> pos = new ArrayList<>(), neg = new ArrayList<>();
    ArrayList<dist> ans = new ArrayList<>();
    for (int i = 0; i < n; i++) {
        if (deb[i] > 0)
            pos.add(new pair<>(i + 1, deb[i]));
        else if (deb[i] < 0)
            neg.add(new pair<>(i + 1, -deb[i]));
    }
    int cpos = 0, cneg = 0;
    while (cpos < pos.size()) {
        long min = Math.min(pos.get(cpos).y, neg.get(cneg).y);
